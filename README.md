...
contains 589 non-empty AP planes because one minimally populated terminal plane
has no corresponding retained Nissl section. The importer removes only that
terminal endpoint and maps all 588 registered sections one-to-one. It does not
interpolate or invent an additional section.
...
7. Inspect representative anterior, septal, hippocampal, midbrain, and
   posterior levels.
