...
WHS export indices 189 through 776. The importer requires exactly one registered
stack plane for every non-empty Paxinos AP plane. It accepts only the target
in-plane shape `(286,409)` or its unambiguous transpose. It does not guess,
crop, or arbitrarily resize manually registered data.
