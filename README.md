...
### Prerelease validation before asset publication

On the registration workstation, the resolver can temporarily use
`G:\nissl_registration`. A different local package can be selected explicitly:
...
This fallback is for release preparation only. Once `download_url` and `sha256`
are populated in the manifest, the published immutable release asset takes
precedence over the workstation-specific path.
...
Paxinos target: **anterior to posterior**. The default is therefore:

```text
PAXINOS_NISSL_STACK_ORDER=anterior-to-posterior
```

This corrects the earlier prerelease test in which Ch. 3 ran posterior to
anterior while the label volume ran anterior to posterior. The setting can be
overridden only for diagnosis:
...
contains 589 non-empty AP planes because the final posterior plane in the
accepted ABBA sequence has no corresponding retained Nissl section. The importer
therefore leaves that final plane empty and maps all 588 registered sections to
the preceding 588 target planes. This matches the manual export and avoids the
one-plane displacement observed in the previous test; no section is interpolated
or invented.
...
archive. From that point onward, a clean checkout no longer depends on
`G:\nissl_registration`.
...
7. Confirm that the final posterior label plane has no Nissl image.
...
Before publication, set `PAXINOS_NISSL_PACKAGE` to a valid local package. After
publication, verify the URL and SHA-256 in the manifest.

### Nissl channel runs in the wrong AP direction

The release default is `anterior-to-posterior`. Ensure an old environment
variable is not overriding it:

```cmd
set PAXINOS_NISSL_STACK_ORDER=
run_builder.bat
```
