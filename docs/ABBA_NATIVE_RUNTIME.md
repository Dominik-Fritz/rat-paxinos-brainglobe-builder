...
`native_abba_renderer.py` materializes the 588 pinned registration planes from
`resources/optional_ch03/whs_nissl_slices_paxinos_40um_ap/` into the
builder-local temporary directory, verifying each against its SHA-256 and
scaling UINT8 to UINT16 by the constant factor 257. These are the images the
saved registration was created against; they are never re-derived from the raw
Waxholm reference volume, because that produced a 22-25 voxel LR displacement
and a ~880 um lateral offset in the atlas. It rewrites all 998 historical loader
