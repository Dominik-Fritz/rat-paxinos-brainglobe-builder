"""Build and install the optional registered WHS/Nissl reference channel.

The final release path consumes the manually registered ABBA ImageJ stack, maps it
to the Paxinos 40-um grid, validates its geometry, and installs it without modifying
the authoritative Paxinos annotation or ontology. Earlier experimental registration
commands remain available for audit and development.
"""
...
SOURCE_PATH = Path(
    os.environ.get(
        "CH03_WHS_NISSL_SOURCE",
        os.environ.get("V53_WHS_NISSL_SOURCE", str(OPTIONAL_DIR / "whs_source_reference.tiff")),
    )
)
WHS_ANNOTATION_PATH = Path(
    os.environ.get(
        "CH03_WHS_ANNOTATION",
        os.environ.get("V53_WHS_ANNOTATION", str(OPTIONAL_DIR / "whs_source_annotation.tiff")),
    )
)
...
BRAINGLOBE_DIR = Path(brainglobe_config.get_brainglobe_dir())
ATLAS_CANDIDATES = [
    ...
    BRAINGLOBE_DIR / "paxinos_watson_rat_40um_v1.0",
    BRAINGLOBE_DIR / "paxinos_watson_rat_40um",
]
REPORT_JSON = REPORT_DIR / "ch03_nissl_report.json"
TARGET_SHAPE = (608, 286, 409)  # AP, SI, LR
TARGET_VOXEL_MM = 0.04
ABBA_IMAGEJ_CANVAS_SHAPE = (656, 940)  # SI, LR in the final ImageJ export
ABBA_IMAGEJ_PIXEL_MM = 0.0195
PERM = (0, 2, 1)
PRE_RESIZE_ROT90 = -1
TARGET_FLIPS = (0,)
...
def status(exit_missing_ok: bool = True) -> int:
    ...
    print("Ch03 WHS/Nissl pipeline status")
...
def create_template() -> int:
    ...
    if SOURCE_PATH.exists():
        ...
        msg = f"WHS/Nissl source is missing; set CH03_WHS_NISSL_SOURCE or place it at {SOURCE_PATH}."
...
def choose_best_auto_oriented_source(fixed_mask: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    for axes, candidate in auto_flip_variants(base):
        ...
        trial = {"extra_flips_after_base_orientation": axes, **score}
        trials.append(trial)
...
def choose_best_label_volume_affine(raw_moving_labels: np.ndarray, fixed_mask: np.ndarray) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    for axes in [(), (0,), (1,), (2,), (0, 1), (0, 2), (1, 2), (0, 1, 2)]:
        ...
        trial = {"extra_flips_after_base_orientation": axes, **metrics, **score}
        trials.append(trial)
...
def load_source_for_auto_affine_context(fixed_mask: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    if matrix is not None:
        vol = load_oriented_source()
        vol = apply_extra_flips(vol, selected.get("extra_flips_after_base_orientation", []))
        return vol, np.asarray(matrix, dtype=np.float64), {"source": "auto_affine_report", "extra_flips_after_base_orientation": selected.get("extra_flips_after_base_orientation", [])}
...
def build_label_landmarks(min_count: int = 12, max_count: int = 160) -> tuple[LabelLandmarkSet, dict, np.ndarray, np.ndarray, Path]:
    ...
    if not WHS_ANNOTATION_PATH.exists():
        raise FileNotFoundError(f"Missing WHS annotation for label-guided registration: {WHS_ANNOTATION_PATH}. Set CH03_WHS_ANNOTATION if needed.")
    fixed_mask = fixed_labels > 0
    source, mat, context = load_source_for_auto_affine_context(fixed_mask)
    extra_flips = context.get("extra_flips_after_base_orientation", [])
    if not extra_flips and isinstance(context.get("source_orientation"), dict):
        extra_flips = ((context["source_orientation"].get("selected") or {}).get("extra_flips_after_base_orientation", []))
...
def run_labels_warp() -> int:
    ...
    # standard workflow while avoiding unsupported local folding. Explicit,
    # reviewed landmarks are required for any later non-linear deformation.
    lm, meta, source, fixed_mask, _ = build_label_landmarks()
    label_mat, fit_metrics = bounded_axis_affine_from_points(lm.moving, lm.fixed, lm.weights)
    mat = np.asarray(meta["mask_affine_matrix_moving_to_fixed"], dtype=np.float64)
...
def run_labels_volume_affine() -> int:
    ...
    fixed_mask = fixed_labels > 0
    if not WHS_ANNOTATION_PATH.exists():
        raise FileNotFoundError(f"Missing WHS annotation for label-volume registration: {WHS_ANNOTATION_PATH}. Set CH03_WHS_ANNOTATION if needed.")
...
def write_ch03_nifti(active: np.ndarray, atlas_dir: Path, target_name: str) -> Path:
    ...
    if annotation_nii.exists():
        ...
        annotation_shape = tuple(int(v) for v in ann_img.shape[:3])
        if annotation_shape == tuple(active.shape):
            nifti_data = active
        elif annotation_shape == (active.shape[2], active.shape[0], active.shape[1]):
            nifti_data = active.transpose(2, 0, 1)
        else:
            raise ValueError(
                f"Cannot orient Ch03 TIFF {active.shape} to annotation NIfTI {annotation_shape} in {atlas_dir}"
            )
    else:
    ...
    img = nib.Nifti1Image(nifti_data.astype(np.uint16, copy=False), affine=affine, header=header)
...
def install_one_ch03_target(atlas_dir: Path, index: int, active: np.ndarray) -> dict:
    ...
    metadata["optional_ch03_registration"] = {
        "installed": True,
        "builder_release": "0.3.0-prerelease",
        ...
        "interpretation": "Manually BigWarp-registered Waxholm Nissl visual aid; Paxinos annotation remains authoritative.",
        "known_limitations": "Different source anatomies are not exactly congruent; deviations may be greater in far anterior/posterior regions.",
        "note": "Optional Ch03 WHS/Nissl reference installed by the reproducible 0.3.0 package build.",
    }
...
def inspect_abba_package(package_path: str) -> int:
    ...
    informative_json = [
        item for item in report["bdv_json_files"]
        if item.get("whs_export_index_count", 0)
        or item.get("thinplate_spline_count", 0)
        or "bdv" in Path(item["path"]).name.lower()
    ]
    print(
        f"JSON details: showing {len(informative_json)} informative BDV/transform files; "
        f"suppressed {len(report['bdv_json_files']) - len(informative_json)} QuPath server/summary files."
    )
    for item in informative_json:
        print(
...
def import_imagej_registered_stack(source_path: str, stack_order: str) -> int:
    ...
    The accepted package contains 588 registered sections. These are mapped to
    the non-empty Paxinos AP sequence after removing its documented unmatched
    terminal endpoint. Native target-grid planes and the calibrated final ABBA
    ImageJ canvas are supported; unknown geometries are rejected.
    """
    ...
    original_stack_shape = tuple(int(v) for v in stack.shape)
    if stack.ndim != 3:
        raise ValueError(f"ImageJ stack must be three-dimensional after squeezing; got {stack.shape}")
    # The accepted registration package contains WHS indices 189..776 (588
    # planes). The annotation mask also contains one minimally populated
    # terminal AP plane for which no registered Nissl section was retained.
    # Excluding that endpoint preserves the documented one-to-one sequence.
    excluded_fixed_ap: list[int] = []
    stack_plane_count = 588 if 588 in stack.shape else None
    if stack_plane_count is not None and fixed_ap.size == stack_plane_count + 1:
        endpoint_counts = np.count_nonzero(fixed_mask[[fixed_ap[0], fixed_ap[-1]]], axis=(1, 2))
        excluded = 0 if endpoint_counts[0] <= endpoint_counts[1] else -1
        excluded_fixed_ap = [int(fixed_ap[excluded])]
        fixed_ap = np.delete(fixed_ap, excluded)
    ...
    spatial_mapping = "native target grid"
    if stack.shape[1:] == TARGET_SHAPE[1:]:
        pass
    elif stack.shape[1:] == TARGET_SHAPE[1:][::-1]:
        stack = stack.transpose(0, 2, 1)
    elif stack.shape[1:] == ABBA_IMAGEJ_CANVAS_SHAPE:
        # ABBA exported the registered canvas at the original WHS in-plane
        # calibration (19.5 um). Sample the central Paxinos field of view at
        # 40 um. This is a calibrated grid conversion, not a new registration.
        source_si = (
            (np.arange(TARGET_SHAPE[1], dtype=np.float64) - (TARGET_SHAPE[1] - 1) / 2)
            * (TARGET_VOXEL_MM / ABBA_IMAGEJ_PIXEL_MM)
            + (stack.shape[1] - 1) / 2
        )
        source_lr = (
            (np.arange(TARGET_SHAPE[2], dtype=np.float64) - (TARGET_SHAPE[2] - 1) / 2)
            * (TARGET_VOXEL_MM / ABBA_IMAGEJ_PIXEL_MM)
            + (stack.shape[2] - 1) / 2
        )
        if source_si[0] < 0 or source_si[-1] > stack.shape[1] - 1 or source_lr[0] < 0 or source_lr[-1] > stack.shape[2] - 1:
            raise ValueError("Calibrated Paxinos field of view lies outside the ABBA ImageJ canvas.")
        ...
        for plane in range(stack.shape[0]):
            converted[plane] = ndimage.map_coordinates(
                stack[plane], [grid_si, grid_lr], order=1, mode="nearest", prefilter=False
            )
        stack = converted
        spatial_mapping = (
            "centered calibrated ABBA ImageJ canvas 19.5um -> Paxinos 40um grid; "
            "manual registration unchanged"
        )
    else:
        raise ValueError(
            f"Registered stack planes have SI/LR shape {stack.shape[1:]}; expected {TARGET_SHAPE[1:]} "
            f"or transpose {TARGET_SHAPE[1:][::-1]}, or the calibrated final ABBA canvas "
            f"{ABBA_IMAGEJ_CANVAS_SHAPE}. Unknown resize/crop operations are disabled."
        )
    ...
    report = {
        ...
        "source_shape": list(original_stack_shape),
        "converted_stack_shape": [int(v) for v in stack.shape],
        ...
        "excluded_terminal_fixed_ap": excluded_fixed_ap,
...
def build_ch03_from_abba_package(package_path: str, stack_order: str) -> int:
    """Inspect, import, and install the final registered ImageJ stack.

    The package remains an immutable provenance source. Exactly one registered
    ImageJ stack must be present; all geometry checks from
    :func:`import_imagej_registered_stack` remain mandatory.
    """
    package = Path(package_path).expanduser().resolve()
    if not package.is_dir():
        raise NotADirectoryError(f"ABBA registration package is missing: {package}")
    inspect_abba_package(str(package))
    candidates = sorted({
        path.resolve()
        for path in package.rglob("*")
        if path.is_file()
        and path.suffix.lower() in {".tif", ".tiff"}
        and "registered" in path.name.lower()
        and "stack" in path.name.lower()
    })
    if len(candidates) != 1:
        raise ValueError(
            "Expected exactly one registered ImageJ TIFF stack in the ABBA package; "
            f"found {len(candidates)}: {[str(path) for path in candidates]}"
        )
    stack = candidates[0]
    print(f"Using registered ImageJ stack: {stack}")
    import_imagej_registered_stack(str(stack), stack_order)
    install_ch03()
    candidate = ROOT / "data" / "output" / "brainglobe_official_candidate" / "paxinos_watson_rat_40um"
    tarball = candidate.parent / f"{candidate.name}.tar.gz"
    if not candidate.is_dir():
        raise FileNotFoundError(f"Official atlas candidate is missing after Ch03 installation: {candidate}")
    manifest_path = candidate / "candidate_manifest.json"
    if manifest_path.exists():
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["metadata"] = json.loads((candidate / "metadata.json").read_text(encoding="utf-8"))
        manifest.setdefault("files", {})["waxholm_anatomy_reference_tiff"] = "waxholm_anatomy_reference.tiff"
        manifest["files"]["waxholm_anatomy_reference_nifti"] = "waxholm_anatomy_reference.nii.gz"
        manifest["status"] = "0.3.0-prerelease-with-registered-nissl"
    ...
    write_json({"ch03_build_from_abba_package": {
        "package": str(package),
        "registered_stack": str(stack),
        "stack_order": stack_order,
        "release_tarball": str(tarball),
        "release_tarball_sha256": tarball_sha256,
        "status": "imported_and_installed",
    }})
    print(f"Repacked official 0.3.0 candidate: {tarball}")
    print("ABBA registration package imported and Ch03 installed successfully.")
    return 0
...
def uninstall_ch03() -> int:
    ...
    Remove the optional Ch03 installation when it is not selected for a release:
        ...
        if isinstance(files, dict):
            for key in [f"{target_name}_tiff", f"{target_name}_nifti"]:
                if key in files:
                    ...
                    changed = True
            metadata["files"] = files
        if metadata.get("optional_ch03_registration") or metadata.get("v53_optional_ch03"):
            metadata.pop("v53_optional_ch03", None)
            metadata["optional_ch03_registration"] = {
                "installed": False,
                "reference_name": target_name,
                "note": "Optional Ch03 reference removed by the explicit uninstall/finalization command.",
            }
            changed = True
        if changed:
...
def finalize_stable() -> int:
    """Finalize a label-only build by withdrawing the optional Ch03 reference."""
    result = uninstall_ch03()
    print("Label-only finalization recorded: the optional Ch03 reference was removed.")
    return result
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Registered WHS/Nissl channel build and audit tools")
    ...
    package_build = sub.add_parser("ch03-build-from-package")
    package_build.add_argument("package_folder")
    package_build.add_argument("stack_order", choices=["anterior-to-posterior", "posterior-to-anterior"])
    ...
    try:
        commands = {
            ...
            "ch03-build-from-package": lambda: build_ch03_from_abba_package(args.package_folder, args.stack_order),
