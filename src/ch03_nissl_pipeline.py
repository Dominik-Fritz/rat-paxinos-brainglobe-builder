...
def import_imagej_registered_stack(source_path: str, stack_order: str) -> int:
    ...
    # planes). During manual registration, the final plane in the ordered
    # Paxinos AP sequence was intentionally left without Nissl. Excluding that
    # exact endpoint preserves the exported one-to-one sequence and prevents a
    # one-plane shift across every retained section.
    ...
    if stack_plane_count is not None and fixed_ap.size == stack_plane_count + 1:
        excluded_fixed_ap = [int(fixed_ap[-1])]
        fixed_ap = fixed_ap[:-1]
    ...
    report = {
        ...
        "excluded_sequence_endpoint": "last (posterior in the accepted ABBA sequence)",
