#!/usr/bin/env python3
"""
V32.10 Build SimpleITK Affine Waxholm Reference Test Atlases
...
- The default V32.10 reference is generated from the low-resolution affine
...
def update_metadata(base_metadata_path: Path, atlas_name: str, target_shape: Sequence[int], reference_stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    meta.update({
        ...
        "reference_strategy": "waxholm_mri_simpleitk_affine_lowres_upsampled_test_reference",
        "status": "diagnostic_test_atlas_not_stable",
        "warning": "Diagnostic test atlas only. Waxholm MRI reference is affine-registered from low-resolution mask registration and upsampled. It is not a final deformable anatomical registration.",
        "version": "1.0",
        "additional_references": [],
        "reference_file": "reference.tiff",
        "annotation_file": "annotation.tiff",
        "hemispheres_file": "hemispheres.tiff",
        "v32_10_note": "Built from V32.9 SimpleITK affine proof-of-concept. Stable paxinos_watson_rat_40um atlas is not modified.",
...
def patch_last_versions(cache_root: Path, atlas_name: str) -> Optional[str]:
    ...
    if last_versions.exists():
        backup_path = cache_root / f"last_versions.conf.backup_v32_10_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def install_to_brainglobe_cache(candidate_dir: Path, atlas_name: str) -> Dict[str, Any]:
    ...
    backup = None
    if cache_dir.exists():
        backup_dir = cache_root / f"_{atlas_name}_backup_v32_10_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def write_text_summary(path: Path, report: Dict[str, Any]) -> None:
    lines: List[str] = []
    lines.append("V32.10 SimpleITK affine Waxholm reference test atlases")
    lines.append("=" * 72)
    ...
    lines.append("- Build separate diagnostic BrainGlobe test atlases from the V32.9 SimpleITK affine path.")
    lines.append("- Stable paxinos_watson_rat_40um is not modified.")
    lines.append("- References are low-resolution affine results upsampled to the target grid; not final deformable registration.")
    lines.append("")
...
def main() -> int:
    ...
    report_dir = ensure_dir(project_root / "reports" / "v32_10_simpleitk_affine_waxholm_reference_test_atlases")
    report: Dict[str, Any] = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "passed": False,
        "project_root": str(project_root),
        "report_dir": str(report_dir),
        "purpose": "Build separate diagnostic BrainGlobe test atlases from V32.9 SimpleITK affine registration proof.",
        "does_modify_stable_atlas": False,
        "does_install_test_atlases": True,
        "reference_generation_mode": "lowres_simpleitk_affine_upsampled_to_target_grid",
        "backend_status": {},
        "errors": [],
        "results": [],
    }
    ...
    try:
        ...
        candidates = [
            {"rank": 1, "name": "rank01", "atlas_name": "paxinos_watson_rat_40um_waxholm_affine_rank01_test", "perm": [2, 1, 0], "flips": [True, True, False], "shift_lowres": [-3, -4, 2]},
            {"rank": 2, "name": "rank02", "atlas_name": "paxinos_watson_rat_40um_waxholm_affine_rank02_test", "perm": [2, 1, 0], "flips": [True, True, True], "shift_lowres": [-3, -4, -2]},
        ]
        ...
        for cand in candidates:
            ...
            # V32.10 diagnostic atlas: upsample affine lowres result into the full target grid.
            ...
            mask_qc_path = report_dir / f"v32_10_{cand['name']}_affine_waxholm_mask_lowres_upsampled_qc.nii.gz"
            save_nifti_uint16(mask_qc_path, affine_mask_full.astype("uint16"), fixed_img)

            preview_path = report_dir / f"v32_10_{atlas_name}_preview.png"
            preview_panel(preview_path, fixed_mask_full, reference_u16, affine_mask_full, f"V32.10 {atlas_name} | SimpleITK affine Waxholm reference test")
        ...
        report["global_conclusion"] = (
            "V32.10 built separate SimpleITK-affine Waxholm reference test atlases for rank01 and rank02. "
            "These are diagnostic ABBA/QC atlases generated from low-resolution affine registration and upsampled to the Paxinos target grid. "
            "Do not promote to stable unless ABBA visual QC confirms usefulness and limitations are documented. "
            "For final anatomical accuracy, a full-resolution affine/deformable registration is still required."
        )
        report["passed"] = True
    except Exception as e:
        report["errors"].append({"error": repr(e), "traceback": traceback.format_exc()})
        report["global_conclusion"] = "V32.10 failed. See JSON errors; no stable atlas was intentionally modified."

    report_path = report_dir / "v32_10_simpleitk_affine_waxholm_reference_test_atlases_report.json"
    report_path.write_text(json.dumps(report, indent=2, default=_json_default), encoding="utf-8")
    summary_path = report_dir / "v32_10_simpleitk_affine_waxholm_reference_test_atlases_summary.txt"
