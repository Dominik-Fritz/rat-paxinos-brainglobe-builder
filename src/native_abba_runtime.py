...
class RuntimePaths:
    ...
    def _project(self) -> Path:
        ...
        return ROOT if self.root == ROOT / "data/native_abba_runtime" else self.root
    ...
    @property
    def brainglobe(self) -> Path: return self._project / "data/brainglobe"
    @property
    def reports(self) -> Path: return self._project / "reports/native_abba"
...
WORK_DIR_PREFIXES = ("native-render-", "native-roundtrip-", "native-geometry-",
                     "native-export-ab-")
...
def release_work_dir(work: Path) -> dict:
    ...
    if not work.exists():
        return {"work_dir": str(work), "removed": True}
    remaining = sum(1 for _ in work.rglob("*") if _.is_file())
    return {"work_dir": str(work), "removed": False, "files_remaining": remaining,
            "note": "left behind (files still open); swept at the start of the next run"}
...
def sweep_stale_work_dirs(temporary: Path, keep: Path | None = None) -> dict:
    ...
    for candidate in temporary.iterdir():
        if candidate == keep or not candidate.is_dir():
            continue
        if not candidate.name.startswith(WORK_DIR_PREFIXES):
            continue
        shutil.rmtree(candidate, ignore_errors=True)
        (failed if candidate.exists() else removed).append(str(candidate))
...
def effective_brainglobe_dir() -> dict:
    ...
    try:
        ...
        resolved = str(config.get_brainglobe_dir())
        source = str(getattr(config, "CONFIG_PATH", "brainglobe config"))
    except Exception as exc:
        return {"effective_brainglobe_dir": None,
                "effective_brainglobe_dir_error": str(exc)}
    return {
        "effective_brainglobe_dir": resolved,
        "effective_brainglobe_dir_source": source,
        "brainglobe_dir_env_is_advisory": True,
        "brainglobe_dir_note": (
            "BRAINGLOBE_DIR in cache_paths is exported for tooling but is not read by "
            "brainglobe-atlasapi; atlases are read from and installed to the path above."
        ),
    }
...
def write_preflight(paths: RuntimePaths, verify_api: bool) -> Path:
    ...
    report.update(effective_brainglobe_dir())
