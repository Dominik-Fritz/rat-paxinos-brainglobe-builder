...
def validate_package(package: Path, manifest: dict) -> tuple[bool, str]:
    ...
    return True, "registration stack and ABBA provenance state found"
...
def resolve(root: Path) -> Path:
    ...
    explicit = os.environ.get("PAXINOS_NISSL_PACKAGE", "").strip()
    embedded = root / "resources" / "optional_ch03" / "abba_registration_package"
    cache_root = root / "data" / "release_assets" / manifest["release"]
    extracted = cache_root / "registration_package"

    candidates: list[tuple[str, Path]] = []
    if explicit:
        candidates.append(("PAXINOS_NISSL_PACKAGE", Path(explicit).expanduser()))
    candidates.append(("embedded package", embedded))
    ...
    url = os.environ.get("PAXINOS_NISSL_ASSET_URL", "").strip() or str(manifest.get("download_url", "")).strip()
    expected_sha = os.environ.get("PAXINOS_NISSL_ASSET_SHA256", "").strip() or str(manifest.get("sha256", "")).strip()
    ...
    if not url:
        ...
        # Transitional fallback for the workstation on which the registration
        # was authored. A published release URL supersedes this local path.
        author_package = Path(r"G:\nissl_registration")
        valid, message = validate_package(author_package, manifest)
        if valid:
            report.update(
                status="ok",
                source="author workstation fallback",
                package=str(author_package.resolve()),
                message=message,
            )
            write_report(root, report)
            return author_package.resolve()
        report.update(
            source="release manifest",
            package=None,
            asset=str(asset),
            message=(
                "No local package was found and the release manifest has no download URL. "
                "Upload the release asset and populate download_url and sha256, or set "
                "PAXINOS_NISSL_ASSET_URL for prerelease testing."
            ),
        )
        write_report(root, report)
