...
def backup_existing_paxinos_cache(bg_dir: Path, clean_install: bool) -> dict[str, Any]:
    ...
    matches = sorted([
        ...
        if p.name.startswith(ATLAS_NAME)
    ], key=lambda p: p.name)
...
def patch_last_versions(bg_dir: Path, clean_install: bool) -> dict[str, Any]:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = bg_dir / f"last_versions.conf.backup_v12_{timestamp}"
    ...
    parser.optionxform = str  # preserve case for other entries, because breaking someone else's cache would be rude even by software standards.
...
def install_versioned_folder(candidate: Path, bg_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "note": "Clean native BrainGlobe cache install created by rat-paxinos-brainglobe-builder V12.",
    }
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    (REPORTS_DIR / "v12_native_install_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    cleanup = report.get("cleanup", {})
    cleanup_lines = [
        "V12 BrainGlobe cache cleanup report",
        "=" * 72,
    ...
    (REPORTS_DIR / "v12_cache_cleanup_report.txt").write_text("\n".join(cleanup_lines), encoding="utf-8")

    lines = [
        "V12 clean native BrainGlobe install report",
        "=" * 72,
        f"Generated: {report['generated_at']}",
    ...
    (REPORTS_DIR / "v12_real_load_report.txt").write_text(text, encoding="utf-8")
    ...
    if report["passed"]:
        ...
        final.append("- Cache duplicates were moved to backup before install.")
        final.append(f"- Exception: {report.get('real_load', {}).get('exception')}")
    (REPORTS_DIR / "v12_final_status.txt").write_text("\n".join(final), encoding="utf-8")

    # Compatibility copies so the old filenames still exist, because habits are clingy.
    (REPORTS_DIR / "v10_final_status.txt").write_text("\n".join(final), encoding="utf-8")
    (REPORTS_DIR / "v10_native_install_report.txt").write_text(text, encoding="utf-8")
...
def main() -> int:
    ...
    table = Table(title="V12 clean native BrainGlobe install")
    ...
    console.print(table)
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v12_final_status.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v12_native_install_report.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v12_cache_cleanup_report.txt'}")
