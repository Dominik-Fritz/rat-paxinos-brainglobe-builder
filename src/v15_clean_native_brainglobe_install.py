...
def patch_last_versions(bg_dir: Path, clean_install: bool) -> dict[str, Any]:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = bg_dir / f"last_versions.conf.backup_v15_{timestamp}"
...
def install_versioned_folder(candidate: Path, bg_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "note": "Clean native BrainGlobe cache install created by rat-paxinos-brainglobe-builder V15.",
    }
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    (REPORTS_DIR / "v15_native_install_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    cleanup = report.get("cleanup", {})
    cleanup_lines = [
        "V15 BrainGlobe cache cleanup report",
        "=" * 72,
    ...
    (REPORTS_DIR / "v15_cache_cleanup_report.txt").write_text("\n".join(cleanup_lines), encoding="utf-8")

    lines = [
        "V15 clean native BrainGlobe install report",
        "=" * 72,
        f"Generated: {report['generated_at']}",
    ...
    text = "\n".join(lines)
    (REPORTS_DIR / "v15_native_install_report.txt").write_text(text, encoding="utf-8")
    (REPORTS_DIR / "v15_real_load_report.txt").write_text(text, encoding="utf-8")
    ...
    (REPORTS_DIR / "v15_final_status.txt").write_text("\n".join(final), encoding="utf-8")
...
def main() -> int:
    ...
    table = Table(title="V15 clean native BrainGlobe install")
    ...
    console.print(table)
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v15_final_status.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v15_native_install_report.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v15_cache_cleanup_report.txt'}")
