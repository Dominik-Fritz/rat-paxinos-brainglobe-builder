...
def ensure_metadata_and_tiffs(folder: Path) -> dict[str, Any]:
    ...
    metadata["description"] = metadata.get("description", "Provisional local BrainGlobe-compatible rat atlas package generated from the Paxinos-Watson digitization included in BlueBrainHeadModels v1.")
    ...
    metadata["additional_references"] = metadata.get("additional_references", [
        {"name": "Paxinos and Watson 2007", "citation": PAXINOS_CITATION, "url": "https://www.elsevier.com/books/the-rat-brain-in-stereotaxic-coordinates/paxinos/978-0-12-374121-9"},
        {"name": "BlueBrainHeadModels v1", "citation": BLUEBRAIN_CITATION, "url": ZENODO_LINK},
    ])
...
def patch_last_versions(bg_dir: Path, clean_install: bool) -> dict[str, Any]:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = bg_dir / f"last_versions.conf.backup_v21_{timestamp}"
...
def install_versioned_folder(candidate: Path, bg_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "note": "Clean native BrainGlobe cache install created by rat-paxinos-brainglobe-builder V21.",
    }
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    (REPORTS_DIR / "v21_native_install_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    cleanup = report.get("cleanup", {})
    cleanup_lines = [
        "V21 BrainGlobe cache cleanup report",
        "=" * 72,
    ...
    (REPORTS_DIR / "v21_cache_cleanup_report.txt").write_text("\n".join(cleanup_lines), encoding="utf-8")

    lines = [
        "V21 clean native BrainGlobe install report",
        "=" * 72,
        f"Generated: {report['generated_at']}",
    ...
    text = "\n".join(lines)
    (REPORTS_DIR / "v21_native_install_report.txt").write_text(text, encoding="utf-8")
    (REPORTS_DIR / "v21_real_load_report.txt").write_text(text, encoding="utf-8")
    ...
    (REPORTS_DIR / "v21_final_status.txt").write_text("\n".join(final), encoding="utf-8")
...
def main() -> int:
    ...
    table = Table(title="V21 clean native BrainGlobe install")
    ...
    console.print(table)
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v21_final_status.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v21_native_install_report.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v21_cache_cleanup_report.txt'}")
