...
def normalize_additional_references(value: Any) -> list[str]:
    ...
    default = [
        PAXINOS_CITATION,
        BLUEBRAIN_CITATION,
        f"Source record: {ZENODO_LINK}",
        f"Associated repository: {GITHUB_LINK}",
    ]
    ...
    if isinstance(value, list):
        ...
        for item in value:
            if isinstance(item, str):
                ...
                parts = []
                for key in ["name", "citation", "url", "doi"]:
...
def fix_metadata(folder: Path) -> dict[str, Any]:
    ...
    metadata["root_id"] = metadata.get("root_id", 997)
    ...
    additional_refs_are_strings = isinstance(metadata.get("additional_references"), list) and all(isinstance(x, str) for x in metadata["additional_references"])
    ...
    return {
        ...
        "additional_refs_are_strings": additional_refs_are_strings,
        "passed": len(missing_after) == 0 and metadata.get("additional_references") == [],
    }
...
def main() -> int:
    ...
    (REPORTS_DIR / f"v22_metadata_compliance_report{suffix}.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V22 metadata compliance report",
        "=" * 72,
        ...
        f"additional_refs_are_strings: {result['additional_refs_are_strings']}",
    ...
    text = "\n".join(lines)
    (REPORTS_DIR / f"v22_metadata_compliance_report{suffix}.txt").write_text(text, encoding="utf-8")
    (REPORTS_DIR / "v22_metadata_compliance_report.txt").write_text(text, encoding="utf-8")
    # compatibility name
    (REPORTS_DIR / "v21_metadata_compliance_report.txt").write_text(text, encoding="utf-8")

    table = Table(title=f"V22 metadata compliance ({args.target})")
    ...
    table.add_row("refs are strings", str(result["additional_refs_are_strings"]))
    table.add_row("missing", str(len(result["missing_after"])))
    console.print(table)
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / f'v22_metadata_compliance_report{suffix}.txt'}")
