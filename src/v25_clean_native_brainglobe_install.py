...
def ensure_metadata_and_tiffs(folder: Path) -> dict[str, Any]:
    ...
    _refs = metadata.get("additional_references")
    ...
    elif isinstance(_refs, str):
        metadata["additional_references"] = [_refs]
    else:
        metadata["additional_references"] = [
            PAXINOS_CITATION,
            BLUEBRAIN_CITATION,
            "Source record: " + ZENODO_LINK,
        ]
...
def backup_existing_paxinos_cache(bg_dir: Path, clean_install: bool, backup_base: Path | None = None) -> dict[str, Any]:
    ...
    if clean_install:
        ...
        for legacy in sorted(bg_dir.glob("_paxinos_cleanup_backup_*"), key=lambda p: p.name):
            target = legacy_root / legacy.name
            if target.exists():
                target = legacy_root / f"{legacy.name}_{timestamp}"
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(legacy), str(target))
            result["legacy_backup_items"].append({"from": str(legacy), "to": str(target)})
...
def patch_last_versions(bg_dir: Path, clean_install: bool) -> dict[str, Any]:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = bg_dir / f"last_versions.conf.backup_v25_{timestamp}"
...
def install_versioned_folder(candidate: Path, bg_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "note": "Clean native BrainGlobe cache install created by rat-paxinos-brainglobe-builder V25.",
    }
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    (REPORTS_DIR / "v25_native_install_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    cleanup = report.get("cleanup", {})
    cleanup_lines = [
        "V25 BrainGlobe cache cleanup report",
        "=" * 72,
    ...
    (REPORTS_DIR / "v25_cache_cleanup_report.txt").write_text("\n".join(cleanup_lines), encoding="utf-8")

    lines = [
        "V25 clean native BrainGlobe install report",
        "=" * 72,
        f"Generated: {report['generated_at']}",
    ...
    text = "\n".join(lines)
    (REPORTS_DIR / "v25_native_install_report.txt").write_text(text, encoding="utf-8")
    (REPORTS_DIR / "v25_real_load_report.txt").write_text(text, encoding="utf-8")
    ...
    (REPORTS_DIR / "v25_final_status.txt").write_text("\n".join(final), encoding="utf-8")
...
def main() -> int:
    ...
    table = Table(title="V25 clean native BrainGlobe install")
    ...
    console.print(table)
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v25_final_status.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v25_native_install_report.txt'}")
    console.print(f"[green]Wrote:[/green] {REPORTS_DIR / 'v25_cache_cleanup_report.txt'}")
