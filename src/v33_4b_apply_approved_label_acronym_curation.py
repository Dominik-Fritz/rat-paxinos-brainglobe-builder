...
REPORT_REL = Path("reports") / "v33_4b_apply_approved_label_acronym_curation"
...
def main() -> int:
    ...
    if errors:
        report["passed"] = False
        (report_dir / "v33_4b_apply_approved_label_acronym_curation_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    (report_dir / "v33_4b_apply_approved_label_acronym_curation_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    (report_dir / "v33_4b_apply_approved_label_acronym_curation_summary.txt").write_text("\n".join(summary) + "\n", encoding="utf-8")
