...
def main() -> int:
    ...
    nissl_requested = not nissl_disabled
    ...
    if reconstruction_report:
        ...
        if alignment.get("median_shift_si_lr_um"):
            si, lr = alignment["median_shift_si_lr_um"]
            within = reconstruction.get("alignment_within_threshold")
            coverage_lines += [
                f"Registered Nissl alignment: {si:+.0f} um SI / {lr:+.0f} um LR "
                f"(guard {alignment.get('warning_threshold_um', 0):.0f} um, "
                f"{'within' if within else 'EXCEEDED'}; measured, nothing shifted)",
            ]
    ...
    if build_failed and nissl_installed:
        nissl_channel_status = "pre-existing; not validated by this failed build"
    elif nissl_failed:
        nissl_channel_status = (
            "RENDER FAILED this build; pre-existing channel retained and unvalidated"
            if nissl_installed else "RENDER FAILED this build; channel not installed"
        )
    elif nissl_disabled:
        nissl_channel_status = "disabled for this build"
    elif nissl_installed:
        nissl_channel_status = "present"
    else:
        nissl_channel_status = "not present"
    lines = [
        ...
        f"Registered Nissl channel: {nissl_channel_status}",
        *([f"Nissl failure code: {native_failure.get('error_code', 'not recorded')}",
           f"Nissl failure message: {native_failure.get('message', 'not recorded')}"]
          if nissl_failed else []),
        ...
        *([f"Nissl reconstruction install status: {install_status}"]
          if install_status != "not recorded" else []),
        ...
        *coverage_lines,
        ...
        ("The current build failed before its Nissl result could be validated; any installed "
         ...
         "WARNING: the optional native Nissl render failed in this build; the Paxinos annotation "
         "was installed without a new Ch03 channel. Any figures above come from this run's "
         "failure record, never from an earlier successful reconstruction." if nissl_failed else
