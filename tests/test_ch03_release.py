...
class NativeRuntimeInitializationTests(unittest.TestCase):
    ...
    def test_successful_render_retires_the_previous_failure_record(self):
        with tempfile.TemporaryDirectory() as temporary:
            report_dir = Path(temporary)
            report_json = report_dir / "ch03_nissl_report.json"
            with mock.patch.object(pipeline, "REPORT_DIR", report_dir), \
                 mock.patch.object(pipeline, "REPORT_JSON", report_json):
                pipeline.write_report({"native_failure": {"error_code": "OLD"}})
                pipeline.write_report({"abba_reconstruction": {"install_status": "installed"}},
                                      drop=("native_failure",))
            written = json.loads(report_json.read_text(encoding="utf-8"))
            self.assertNotIn("native_failure", written)
            self.assertEqual(written["abba_reconstruction"]["install_status"], "installed")
        source = (Path(__file__).parents[1] / "src/native_abba_renderer.py").read_text(
            encoding="utf-8"
        )
        self.assertIn('drop=("native_failure",)', source)

    def test_stale_work_dirs_are_swept_but_foreign_entries_are_left_alone(self):
        from src import native_abba_runtime as runtime
        with tempfile.TemporaryDirectory() as temporary:
            tmp = Path(temporary)
            stale = tmp / "native-render-old"
            current = tmp / "native-render-current"
            foreign = tmp / "hsperfdata_49152"
            unrelated = tmp / "somebody-elses-data"
            for folder in (stale, current, foreign, unrelated):
                folder.mkdir()
                (folder / "plane.tiff").write_bytes(b"0")
            result = runtime.sweep_stale_work_dirs(tmp, keep=current)
    ...
    def test_retained_work_dir_is_reported_instead_of_silently_ignored(self):
        from src import native_abba_runtime as runtime
        with tempfile.TemporaryDirectory() as temporary:
            work = Path(temporary) / "native-render-x"
            work.mkdir()
            (work / "plane.tiff").write_bytes(b"0")
            self.assertTrue(runtime.release_work_dir(work)["removed"])

            blocked = Path(temporary) / "native-render-y"
            blocked.mkdir()
            (blocked / "plane.tiff").write_bytes(b"0")
            with mock.patch.object(runtime.shutil, "rmtree", lambda *a, **k: None):
                result = runtime.release_work_dir(blocked)
            self.assertFalse(result["removed"])
            self.assertEqual(result["files_remaining"], 1)

    def test_write_nifti_validates_orientation_and_separates_source_from_target(self):
        import numpy as np
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            atlas.mkdir()
            stage.mkdir()
            reference = np.zeros((4, 3, 2), dtype=np.uint16)
            nib.save(nib.Nifti1Image(reference, np.eye(4)), atlas / "annotation.nii.gz")

            written = pipeline.write_nifti(np.ones((4, 3, 2), dtype=np.uint16),
                                           atlas, stage, "channel")
    ...
    def test_install_stages_nifti_through_the_validated_writer(self):
        source = (Path(__file__).parents[1] / "src/ch03_nissl_pipeline.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("write_nifti(active, atlas, stage_dir, name)", source)
        self.assertNotIn('(stage_dir / "annotation.nii.gz").exists()', source)
    ...
    def test_report_is_written_atomically(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(pipeline, "REPORT_DIR", report_dir), \
                 mock.patch.object(pipeline, "REPORT_JSON", report_json):
                pipeline.write_report({"first": 1})
                pipeline.write_report({"second": 2})
            written = json.loads(report_json.read_text(encoding="utf-8"))
            self.assertEqual(written["first"], 1)
            self.assertEqual(written["second"], 2)
            self.assertEqual(list(report_dir.glob("*.partial")), [])

    def test_preflight_reports_the_directory_brainglobe_actually_uses(self):
        from src import native_abba_runtime as runtime
        resolved = runtime.effective_brainglobe_dir()
        # BRAINGLOBE_DIR is exported but ignored by brainglobe-atlasapi; the
        # report must name the path that really holds the atlases.
        self.assertTrue(resolved["brainglobe_dir_env_is_advisory"])
        self.assertTrue(resolved["effective_brainglobe_dir"])
        self.assertIn("not read by", resolved["brainglobe_dir_note"])

    def test_failing_rollback_never_masks_the_original_install_error(self):
        # Snapshot dir and installed atlas can live on different volumes. A
        # rollback that cannot restore must not replace the real cause.
        import numpy as np
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            atlas = root / "atlas"
            atlas.mkdir()
            (atlas / "metadata.json").write_text("{}", encoding="utf-8")
            for name in ("annotation.tiff", "annotation.nii.gz"):
                (atlas / name).write_bytes(b"0")
            reports = root / "reports"
            reports.mkdir()
            report = {"renderer_backend": "native_abba_0.11",
                      "native_backend_verified": True, "visual_parity_status": "pending"}
            real_copy = pipeline.shutil.copy2

            def copy_that_fails_only_when_restoring(source, destination):
                if Path(source).parent.parent.name.startswith("ch03-install-transaction-"):
                    raise OSError(17, "cannot move the file to a different disk drive")
                return real_copy(source, destination)

            with mock.patch.object(pipeline, "REPORT_DIR", reports), \
                 mock.patch.object(pipeline, "atlas_candidates", lambda: [atlas]), \
                 mock.patch.object(pipeline.tifffile, "imread",
                                   lambda path: np.zeros((2, 2, 2), dtype=np.uint16)), \
                 mock.patch.object(pipeline, "_transactional_atlas_install",
                                   mock.Mock(side_effect=RuntimeError("real install failure"))), \
                 mock.patch.object(pipeline.shutil, "copy2",
                                   copy_that_fails_only_when_restoring):
                with self.assertRaisesRegex(RuntimeError, "real install failure"):
                    pipeline.install_channel(report)

    def test_install_rollback_restores_by_copy_not_cross_drive_replace(self):
        source = (Path(__file__).parents[1] / "src/ch03_nissl_pipeline.py").read_text(
            encoding="utf-8"
        )
        rollback = source.split("rollback_failures = []", 1)[1].split("raise", 1)[0]
        self.assertIn("shutil.copy2(backup, destination)", rollback)
        self.assertNotIn("os.replace(backup, destination)", rollback)
        self.assertIn("CH03_ROLLBACK_INCOMPLETE", source)
