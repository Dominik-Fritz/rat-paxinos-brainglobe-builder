...
def add_brainglobe_atlases(ij):
    ...
    # V17 local BrainGlobe atlas visibility patch
    try:
    ...
    class AtlasSupplier(object):
        ...
        def get(self):
            try:
                ...
                log_dir = Path(os.environ.get("USERPROFILE", ".")) / "abba_paxinos_debug"
                log_dir.mkdir(parents=True, exist_ok=True)
                log_file = log_dir / (str(self.atlas_name) + "_supplier_get_error.log")

                def append_log(text):
                    previous = ""
                    if log_file.exists():
                        previous = log_file.read_text(encoding="utf-8", errors="replace")
                    log_file.write_text(previous + text + "\\n", encoding="utf-8")

                append_log("ENTER AtlasSupplier.get")
                append_log("atlas_name=" + str(self.atlas_name))

                bg_atlas = BrainGlobeAtlas(self.atlas_name)
                append_log("BrainGlobeAtlas OK")
                append_log("root_dir=" + str(bg_atlas.root_dir))

                from abba_python.abba_atlas import AbbaAtlas
                append_log("AbbaAtlas import OK")

                current_atlas = AbbaAtlas(bg_atlas, self.ij)
                append_log("AbbaAtlas constructor OK")

                current_atlas.initialize(None, None)
                append_log("initialize OK")

                Abba.opened_atlases[self.atlas_name] = current_atlas
                append_log("AtlasSupplier.get OK")
                ...
                print("ABBA SUPPLIER DEBUG LOG: " + str(log_file))
    ...
    def __init__(
        ...
        if atlas_name not in Abba.opened_atlases:
            ...
            if atlas_name == 'Adult Mouse Brain - Allen Brain Atlas V3p1':
                ...
                # V23 ABBA BrainGlobe atlas debug patch
                try:
